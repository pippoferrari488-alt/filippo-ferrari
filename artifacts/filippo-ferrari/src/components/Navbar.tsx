  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled ? "nav-glass shadow-lg shadow-black/30 border-b border-white/5" : "bg-transparent"
      }`}
    >
          ...
          <nav className="hidden md:flex items-center gap-1">
            {navLinks.map((l) => (
              <Link key={l.href} href={l.href}>
                <span
                  className={`px-3.5 py-2 rounded text-sm font-medium tracking-wide transition-all duration-200 cursor-pointer ${
                    location === l.href
            ...
            <Link href="/contatti">
              <span className="ml-4 px-5 py-2.5 btn-red rounded text-sm font-semibold cursor-pointer">
                Parliamone
              </span>
            </Link>
          </nav>
